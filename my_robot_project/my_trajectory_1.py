...
###################################################### 2. ROS2服務回呼
    def enter_srv_callback(self, request, response):
        self.get_logger().info('\033[1;32m%s\033[0m' % "進入記錄軌跡模式") 
    ...
    def exit_srv_callback(self, request, response):
        if self.enter:
            self.get_logger().info('\033[1;32m%s\033[0m' % "結束記錄軌跡模式")
    ...
    def start_srv_callback(self, request, response):
        if request.data:
            if self.app:
                target_list = []
                target_list.extend(list(TARGET_CLASSES['weld_point']))
                target_list.extend(list(TARGET_CLASSES['board']))
                self.target_list = target_list
                self.target_list_temp = copy.deepcopy(self.target_list)
            self.send_request(self.start_yolov8_client, Trigger.Request())
            self.get_logger().info('\033[1;32m%s\033[0m' % "開始記錄軌跡")
            pick_and_place.interrupt(False)
            self.enable_transport = True
            response.message = "start"
        else:
            self.send_request(self.stop_yolov8_client, Trigger.Request())
            self.get_logger().info('\033[1;32m%s\033[0m' % "暫停記錄軌跡")
            pick_and_place.interrupt(True)
            self.enable_transport = False
    ...
    def set_target_srv_callback(self, request, response):
        ...
        for i in request.data:
            target_list.extend(list(TARGET_CLASSES[i]))
...
###################################################### 5. 核心執行緒邏輯
                            ...
                            if result is not None:
                                if self.last_position is not None:
                                    ...
                                    if self.count_still > 10:
                                        ...
                                        self.transport_info = [position, yaw, target_category]
    ...
    def go_home(self, interrupt=True):      #回到自訂初始位置
        """
    ...
    def calculate_move_distance(self, position1, position2):
        distance = math.sqrt(pow(position1[0] - position2[0], 2) + pow(position1[1] - position2[1], 2))
        return distance


def main():
    node = MyTrajectory('mytrajectory')
