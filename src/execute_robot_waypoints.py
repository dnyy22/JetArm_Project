    ...
    try:
        ...
        for lift_z in lift_levels:
            ...
            if not valid:
                ...
                break
            initial_lift.append((pulses, args.step_duration))
        ...
        for row in selected:
            ...
            for label, xyz, duration in levels:
                ...
                solved.append((label, pulses, duration))
        ...
        for pulses, duration in initial_lift:
            node.publish_pulses(pulses, duration)
            # Do not overwrite an in-flight servo command.  Waiting for the
            # commanded duration is what keeps these increments vertical.
            time.sleep(duration + args.transition_pause)
        if not args.yes:
        ...
        time.sleep(args.travel_duration + args.transition_pause)
        ...
        for row, solved in plans:
            ...
            for index, (label, pulses, duration) in enumerate(solved):
            ...
            for label, pulses, duration in reversed(solved[:-1]):
