...
from my_arm_control.yolov8_trt import YoLov8TRT,plot_one_box
...
class Yolov8Node(Node):
    def __init__(self, name):
        ...
        model_dir = os.path.join(get_package_share_directory('my_arm_control'), 'models')
        plugin_path = lib if os.path.isabs(lib) else os.path.join(model_dir, lib)
        engine_path = engine if os.path.isabs(engine) else os.path.join(model_dir, engine)
        ctypes.CDLL(plugin_path)
        self.yolov8_wrapper = YoLov8TRT(engine_path)
